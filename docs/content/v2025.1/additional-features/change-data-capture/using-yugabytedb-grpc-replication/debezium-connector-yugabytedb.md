...
| log.commit.offset.interval.ms | 600000 | Interval in milliseconds at which the connector logs the commit offset map (tablet ID to OpId) at INFO when Kafka Connect commits offsets, for both streaming and snapshot. Set to `-1` to disable. Entries with key `transaction_id` are omitted. With very large tablet counts, each log cycle can emit many lines; increase the interval or use `-1` if needed. <br/><br/> **Note:** This flag is only available in `TBD` and later. |
